...
LIBS = -shared -Wl,-soname,libarray_hash.so -ldl -L. \
       -L"$(SCIDB_THIRDPARTY_PREFIX)/3rdparty/boost/lib" -L"$(SCIDB)/lib" \
       -Wl,-rpath,$(SCIDB)/lib:$(RPATH)

SRCS = LogicalArrayHash.cpp \
       PhysicalArrayHash.cpp
...
all: libarray_hash.so

clean:
	rm -rf *.so *.o

libarray_hash.so: $(SRCS) ValueHasher.h
	@if test ! -d "$(SCIDB)"; then echo  "Error. Try:\n\nmake SCIDB=<PATH TO SCIDB INSTALL PATH>"; exit 1; fi
	$(CXX) $(CCFLAGS) $(INC) -o LogicalArrayHash.o -c LogicalArrayHash.cpp
	$(CXX) $(CCFLAGS) $(INC) -o PhysicalArrayHash.o -c PhysicalArrayHash.cpp
	$(CXX) $(CCFLAGS) $(INC) -o libarray_hash.so plugin.cpp LogicalArrayHash.o PhysicalArrayHash.o $(LIBS)
	@echo "Now copy libarray_hash.so to $(INSTALL_DIR) on all your SciDB nodes, and restart SciDB."
